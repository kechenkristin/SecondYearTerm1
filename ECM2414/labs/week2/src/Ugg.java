...
public class Ugg {
    ...
    public static void main(String[] args) {
        final Ugg ugg = new Ugg();


        new Thread(() -> {
            for (int i = 0; i < 20; i++) {
                UggRock uggRock = ugg.drawRock();
                System.out.println(Thread.currentThread().getName() + " thread is drawing" +
                        "a uggRock with " + uggRock.size + " size " + uggRock.type + " type ");
            }
        }, "t1").start();
